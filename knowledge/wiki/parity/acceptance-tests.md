...
## Геометрия продольных осей

Unit-тесты фиксируют схему здания 38 м с пятью центральными пролётами по 6 м:
торцевые пролёты по 4 м, восемь осей, шесть внутренних рам. Отдельно проверяется,
что количество ферм берётся из внутренних осей, а разрезные прогоны делятся на
партии длиной 4 и 6 м без изменения общей массы. Это проверка логистики и количества,
а не подтверждение расчётных формул Excel-oracle.
...
- для подкрановой балки решить, использовать ли `B19` или `D19` как итоговое значение шага рёбер в ведомости; UI уже различает эти поля;
